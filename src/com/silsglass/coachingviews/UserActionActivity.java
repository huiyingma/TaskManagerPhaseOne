...
public class UserActionActivity extends Activity {
	...
	private void showItems(List<String> items) {
		...
		for (String item : items) {
			Log.d(TAG, "item: " + item);
			allItems += item + " ";
			if (item.toLowerCase().contains("take picture")) {
				mTextView
						.setText("here I should allow to take a picture, tap and I'll listen again, and try not to get distracted]");
				Intent i = new Intent(android.provider.MediaStore.ACTION_IMAGE_CAPTURE);
				startActivityForResult(i, CAPTURE_IMAGE_ACTIVITY_REQUEST_CODE);
				return;
			} 
			if (item.toLowerCase().contains("take video")) {
				mTextView
						.setText("here I should allow to tape a video, tap and I'll listen again, and try not to get distracted]");
				Log.d(TAG, "bitcoin was spoken");
				Intent i = new Intent(this, BasicVideoCaptureActivity.class);
				startActivity(i);  
				return;
			} 
			if (item.toLowerCase().contains("take audio")) {
				mTextView
						.setText("here I should allow to record audio, tap and I'll listen again, and try not to get distracted]");
				Log.d(TAG, "bitcoin was spoken");
				return;
			} 
		}
