 ...
 */
public class TutorialActivity extends BaseTemplateTask {
    ...
    protected TaskCoachingModel createTaskCoachingModel() {
        ...
        return new TaskCoachingModel(tutorialPhrases);
    }
    ...
    protected void handleRunGesture(Gesture gesture) {
        int phraseIndex = getTaskCoachingModel().getCurrentPhraseIndex();
        switch (gesture) {
            case TAP:
        ...
        if (phraseIndex == getTaskCoachingModel().getPhraseCount() - 1) {
            finish();
        }
    }
}
